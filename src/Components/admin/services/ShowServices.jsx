...
import Example from "../../loading/loadiing";
const styles = (theme) => ({
	root: {
		flexGrow: 1,
	},
...
class ServicesViewAdmin extends React.Component {
	...
	componentDidMount() {
		Axios({
			url: "http://localhost:5000/Digital_Saloon.com/api/recomended_services",
			method: "GET",
			headers: {
			...
			<React.Fragment className={classes.root}>
				...
				<Grid container spacing={3} maxWidth="lg">
					{this.state.List_of_services.length === 0 && (
						<Grid item xs={6}>
							<Example type="balls" color="blue" />
							<h2 style={{ textAlign: "center" }}>No recomended service</h2>
						</Grid>
					)}
					{this.state.List_of_services.length !== 0 &&
